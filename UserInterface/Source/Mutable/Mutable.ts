      ...
      for (let key in mutator) {
        if (this.ui.querySelector("#" + key) != null) {
          ...
          if (type instanceof Object) {
            let selectElement: HTMLSelectElement = <HTMLSelectElement>_ui.querySelector("#" + key);
            selectElement.value = <string>mutator[key];
          }
          else {
            let element: HTMLElement = _ui.querySelector("#" + key);
      ...
      for (let key in mutator) {
        if (this.ui.querySelector("#" + key) != null) {
          ...
          if (type instanceof Object) {
            ...
            switch (type) {
              case "Boolean":
                let checkbox: HTMLInputElement = <HTMLInputElement>_ui.querySelector("#" + key);
                checkbox.checked = <boolean>mutator[key];
                break;
              case "String":
                let textfield: HTMLInputElement = <HTMLInputElement>_ui.querySelector("#" + key);
                textfield.value = <string>mutator[key];
                break;
              case "Number":
                let stepper: HTMLInputElement = <HTMLInputElement>_ui.querySelector("#" + key);
                if (document.activeElement != stepper) {
                  stepper.value = <string>mutator[key];
                }
                break;
              default:
                let fieldset: HTMLFieldSetElement = <HTMLFieldSetElement>_ui.querySelector("#" + key);
