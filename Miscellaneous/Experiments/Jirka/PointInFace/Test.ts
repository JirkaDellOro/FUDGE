namespace PointInFace {
  ...
  function start(_event: Event): void {
    vertices = new ƒ.Vertices(
      ...
      new ƒ.Vertex(new ƒ.Vector3(400, 500, 0)),
      new ƒ.Vertex(new ƒ.Vector3(700, 100, 0))
    );
  ...
  function hndMouse(_event: MouseEvent): void {
    ...
    for (let vertex of vertices) {
      ...
      let diff: ƒ.Vector3 = ƒ.Vector3.DIFFERENCE(vertex.position, mouse);
      diffs.push(diff);
    }
    ...
    let n0: ƒ.Vector3 = ƒ.Vector3.CROSS(diffs[1], diffs[0]);
    let n1: ƒ.Vector3 = ƒ.Vector3.CROSS(diffs[2], diffs[1]);
    let n2: ƒ.Vector3 = ƒ.Vector3.CROSS(diffs[0], diffs[2]);

    let dot1: number = ƒ.Vector3.DOT(n0, n1);
    let dot2: number = ƒ.Vector3.DOT(n0, n2);
