...
(function (IntervalSolver) {
    var ƒ = FudgeCore;
    var ƒAid = FudgeAid;
    let arInSoNumber = new ƒAid.ArithIntervalSolver((_num) => { return _num < 3; }, (_left, _right) => { return (_left + _right) / 2; }, (_left, _right, _epsilon) => { return Math.abs(_right - _left) < _epsilon; });
    arInSoNumber.solve(0, 10, 0.001);
    console.log(arInSoNumber.toString());
    let rect = new ƒ.Rectangle(10, 10, 100, 100);
    let arInSoV2 = new ƒAid.ArithIntervalSolver((_v) => { return rect.isInside(_v); }, (_left, _right) => {
