...
(function (ScenesForUi) {
    ...
    function createAxisCross() {
        ...
        let cubeRed = ScenesForUi.createCompleteMeshNode("Red", mtrRed, meshCube);
        let cubeGreen = ScenesForUi.createCompleteMeshNode("Green", mtrGreen, meshCube);
        let cubeBlue = ScenesForUi.createCompleteMeshNode("Blue", mtrBlue, meshCube);
    ...
    ScenesForUi.createAxisCross = createAxisCross;
    function createArrow(_name, _color) {
        ...
        let shaft = ScenesForUi.createCompleteMeshNode("Shaft", material, meshCube);
        let head = ScenesForUi.createCompleteMeshNode("Head", material, meshPyramid);
    ...
    ScenesForUi.createCoordinateSystem = createCoordinateSystem;
    function createThreeLevelNodeHierarchy() {
        createMiniScene();
        let child = ScenesForUi.node.getChildren()[0];
        let grandchild;
    ...
    ScenesForUi.createThreeLevelNodeHierarchy = createThreeLevelNodeHierarchy;
    function createMiniScene() {
        ƒ.RenderManager.initialize();
        ScenesForUi.node = createCompleteMeshNode("Node", new ƒ.Material("Red", ƒ.ShaderUniColor, new ƒ.CoatColored(new ƒ.Color(1, 0, 0, 1))), new ƒ.MeshCube());
        let cmpTransform = ScenesForUi.node.cmpTransform;
        cmpTransform.local.scaleX(2);
        ScenesForUi.camera = createCamera();
        let child = new ƒ.Node("Child");
        ScenesForUi.node.appendChild(child);
    }
    ScenesForUi.createMiniScene = createMiniScene;
    function createViewport(_canvas = null) {
        if (!_canvas) {
            _canvas = document.createElement("canvas");
            document.body.appendChild(_canvas);
        }
        ScenesForUi.viewPort = new ƒ.Viewport();
        ScenesForUi.viewPort.initialize("TestViewport", ScenesForUi.node, ScenesForUi.camera.getComponent(ƒ.ComponentCamera), _canvas);
        // viewPort.drawScene();
        ScenesForUi.viewPort.showSceneGraph();
    }
    ScenesForUi.createViewport = createViewport;
    ...
    ScenesForUi.createCamera = createCamera;
    ...
    ScenesForUi.createCompleteMeshNode = createCompleteMeshNode;
    ...
    ScenesForUi.dollyViewportCamera = dollyViewportCamera;
})(ScenesForUi || (ScenesForUi = {}));
