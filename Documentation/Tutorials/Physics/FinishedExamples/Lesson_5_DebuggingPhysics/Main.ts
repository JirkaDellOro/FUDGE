...
namespace Tutorials_FUDGEPhysics_Lesson1 {
    ...
    */
    ...
    function init(_event: Event): void {
        ...
        bodies[1] = createCompleteNode("JointHolder_Cylindrical", fixedJointMaterial, new f.MeshCube(), 0, f.BODY_TYPE.STATIC);
        ...
        bodies[2] = createCompleteNode("Connected_Cylindrical", jointMaterial, new f.MeshCube(), 1, f.BODY_TYPE.DYNAMIC);
        bodies[2].mtxLocal.translate(new f.Vector3(2, 2, 0));
        bodies[2].mtxLocal.scale(new f.Vector3(0.3, 2, 0.3));
        hierarchy.appendChild(bodies[2]);

        bodies[3] = createCompleteNode("Pyramid", defaultMaterial, new f.MeshPyramid(), 1, f.BODY_TYPE.DYNAMIC, f.COLLISION_GROUP.DEFAULT, f.COLLIDER_TYPE.PYRAMID);
        ...
        cylindricalJoint = new f.JointCylindrical(bodies[1].getComponent(f.ComponentRigidbody), bodies[2].getComponent(f.ComponentRigidbody));
        cylindricalJoint.internalCollision = true;
        cylindricalJoint.maxMotor = 0.25;
        cylindricalJoint.minMotor = -0.25;
        cylindricalJoint.rotorSpeed = 5;
        cylindricalJoint.rotorTorque = 25;
    ...
    function hndKeydown(_event: KeyboardEvent): void {
        ...
        */
        if (_event.code == f.KEYBOARD_CODE.Y) { //Y == Z on german keyboards - Go through the different Physics Debug Modes
            let currentMode: number = viewPort.physicsDebugMode; //Get current mode
            currentMode = currentMode == 5 ? 0 : viewPort.physicsDebugMode += 1; //Go one mode further until the last mode is reached then reset
            viewPort.physicsDebugMode = currentMode; //Tell the physics settings to go to that mode
        }
