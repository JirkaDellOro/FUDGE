namespace FudgeCore {
  export class AnimationSprite extends Animation {
    ...
    constructor(_name: string = "AnimationSprite") { //}, _fps: number = 15) {
      super(_name, {}, 15);
      this.create(this.frames, this.wrapAfter, this.start, this.size, this.next, this.wrap);
    }

    public create(_frames: number, _wrapAfter: number, _start: Vector2, _size: Vector2, _next: Vector2, _wrap: Vector2) {
      this.frames = _frames;
